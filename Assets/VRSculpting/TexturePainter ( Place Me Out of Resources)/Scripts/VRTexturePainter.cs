...
namespace sandiegoJohn.VRsculpting{
	...
	public class VRTexturePainter : MonoBehaviour {
		...
		VRTK_ControllerEvents controllerTrigger;
		public GameObject rightContr;
		...
		int brushCounter=0,MAX_BRUSH_COUNT=5000; //To avoid having millions of brushes
		bool saving=false; //Flag to check if we are saving the texture

		public Transform rayPos;
		public bool pressingTrigger = false;

		public Slider cursorSize;

		public Button Erase;
		private bool eraserIsActive = false;

		void Start(){
			//rightContr = GameObject.FindGameObjectWithTag ("controllerRight");
		}

		void OnEnable(){

			controllerTrigger = rightContr.GetComponent<VRTK_ControllerEvents>();
			controllerTrigger.TouchpadPressed += ControllerTrigger_TriggerClicked;
			controllerTrigger.TouchpadReleased += ControllerTrigger_TriggerUnclicked;
		...
		void FixedUpdate () {
			...
			if (pressingTrigger || Input.GetKey(KeyCode.P)) {
				//Debug.Log ("should be painting");
				Paint();
				brushSize = cursorSize.value;
			}
			UpdateBrushCursor ();
		}
		...
		bool HitTestUVPosition(ref Vector3 uvWorldPosition){
			...
			Ray cursorRay= new Ray(rayPos.transform.position, rayPos.transform.forward);
			if (Physics.Raycast(cursorRay,out hit,200f)){
		...
		void ControllerTrigger_TriggerClicked (object sender, ControllerInteractionEventArgs e)
		{
		...
		//#if !UNITY_WEBPLAYER 
			IEnumerator SaveTextureToFile(Texture2D savedTexture){		
				brushCounter=0;
				string fullPath=System.IO.Directory.GetCurrentDirectory()+"\\Assets/Resources\\";
				System.DateTime date = System.DateTime.Now;
