...
namespace sandiegoJohn.VRsculpting{
	...
	public class TexturePainter : MonoBehaviour {
		...
		public Transform rayPointPosition;
		...
		void Update () {
			...
			brushColor = Color.green;	//Updates our painted color with the selected color
			if (PMD.isTriggerOn) {
				DoAction();
				Debug.Log ("doing action");
			}

			//Debug.DrawRay (rayPointPosition.transform.position, rayPointPosition.forward, Color.green);
		}
		...
		bool HitTestUVPosition(ref Vector3 uvWorldPosition){
			RaycastHit hit;
			//Vector3 cursorPos = new Vector3 (PMD.rightController.transform.position.x, PMD.rightController.transform.position.y, 0.0f);
			Ray cursorRay = new Ray(PMD.rightController.transform.position, -PMD.rightController.transform.up);
			if(Physics.Raycast(cursorRay, out hit, 5f)){
