...
//				refCamera = Camera.main;
		...
		void Update () {
			if (PMD.currentTool == Tools.Paint) {
				UserInputUpdate ();
				//Debug.Log ("painting");
			}
		}
		...
		void UserInputUpdate(){
			//Vector3 cursorPos = new Vector3 (PMD.hit.transform.position.x, PMD.hit.transform.position.y, PMD.hit.transform.position.z);
			Ray cursorRay = new Ray(raypos.transform.position, raypos.transform.forward);
			//Debug.Log("in color selector");
			RaycastHit hit = new RaycastHit ();
			if(Physics.Raycast(cursorRay, out hit, 100f)){
				Vector3 localPosition = hit.point;
				//Debug.Log (PMD.hit.point);
				float dist = Vector2.Distance(Vector2.zero,localPosition);
				//Debug.Log ("dist: " + dist + " locationpos: "+ localPosition);
				if(dist>0.22)
